...
When the defect fixer has acknowledged the defect is valid, the defect fix development process begins. The objective of the process is to ensure that the developed fix is correctly tracked in the defect tracking system and checked-in to Configuration Management (CM) system.
...
|                                      |   appropriate branch of the CM system.|
|                                      | - Defect fixer has tested the fix.    |
+--------------------------------------+---------------------------------------+
| **Outputs**                          | - Defect has been fixed with State    |
|                                      |   set to "Checked In", and the        |
|                                      |   details of the fix recorded.        |
|                                      | - The defect fix ready to be          |
|                                      |   integrated.                         |
+--------------------------------------+---------------------------------------+
...
#. **Optionally, consider alternative solutions, select the appropriate one, and estimate the effort.**
   ...
   -  Estimate may be captured in the *Estimated Effort* field.
...
#. When the defect is fixed, the state will be moved to "Checked-In".   
...
-  If the defect is to be resolved in an external component, such as Open Source or other component that is not directly under our control, the defect status is set to "On-hold"

-  The defect is then fixed in the external (non-product) location.
...
+--------------+-------------------------+---------------+-------------------------+------------------------------------------------------------------+
| 08/25/2020   | N/A                     | 0.3           | Shree Vidya Jayaraman   | Updated the links based on Doina and Rodger's feedback           |
+--------------+-------------------------+---------------+-------------------------+------------------------------------------------------------------+
|              |                         |               |                         |                                                                  |
+--------------+-------------------------+---------------+-------------------------+------------------------------------------------------------------+
