...
+--------------------------------------+-------------------------------------------------------------------------+
| **Entry Criteria**                   | - Epics & Stories/requirements are available                            |
|                                      | - Software design documents (HLDs), as needed, have been created        |
|                                      | - Test code checked in and Feature Verification Testing completed       |
+--------------------------------------+-------------------------------------------------------------------------+
| **Inputs**                           | - Release milestones                                                    |
|                                      | - Epics and/or Stories needed                                           |
|                                      | - Design documents, as needed                                           |
|                                      | - Software in integrated into the CM system and ready for release       |
|                                      |   verification                                                          |
+--------------------------------------+-------------------------------------------------------------------------+
| **Exit Criteria**                    | - Execution of tests                                                    |
|                                      | - Review and analysis of test results                                   |
+--------------------------------------+-------------------------------------------------------------------------+
| **Outputs**                          | - Release Test Strategy                                                 |
|                                      | - Test review artifacts, as needed                                      |
...
.. list-table::
   ...
   * - 1
     ...
     - Develop a strategy for verifying the integrated software taking into consideration the categorization (e.g., software, component, function and design) of the software requirements and the requirements of any standard(s), if applicable. 
       ...
       - For each release, a Release Test Strategy document is created using the `Release Test Strategy template <../../../ProcessDocuments/CoreDev/Verification/ReleaseTestStrategyTemplate.docx>`__
       - For other testing guidance, refer to `Test Process Guideline <./TestProcessGuideline.html>`__
	   
       The release test strategy outlines the end-to-end product verification cycle that occurs over the course of an entire release.  It details the set of configurations used for regression, performance, and footprint tests.  It also details the validate workflows for customer uses case & out-of-box experiences.
   ...
   * - 2
     ...
     - The release test process encompasses different categories of tests (e.g., regression, system, performance, footprint, out-of-box experience, etc.).
	 
       As part of release testing, new test cases may be developed with the goal of demonstrating compliance of the software to required workflows and experiences through verification tests.
       ...
       - Reviewing and analyzing customer use cases and desired workflows
   ...
   * - 4
     ...
     - Verification tests are reviewed and the reviewer examines the test code for defects. 
       ...
       In the case that agreement cannot be reached, the issue is escalated to the Test Lead.
   ...
   * - 5
     ...
     - Verify the software using test cases that the software fulfilled its intended use. Verification/Test is performed according to the test cases. 
       ...
       Test results are uploaded to the Test Management System.
   ...
   * - 7
     ...
     - Once completed, the test code and review artifacts are delivered by checking them into the CM.
       ...
       Engineering Program Manager (EPM) tracks all issues to closure and manages project activities toward release.

|

Related Process Assets/Tools
----------------------------
- Verification Validation Test Summary Flow Diagram
...
.. |image0| image:: ../../../_static/CoreDev/Verification/ReleaseVerification.jpg